...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    const errorResponseError =
      error.response?.data?.error || 'No error provided'; //  custom error message added by all modules
    ...
    if (
      ...
      debugLog(
        `1. errorCode=${errorCode} || errorResponseCode=${errorResponseCode} || errorAxiosMessage=${errorAxiosMessage}`
      );
    ...
    if (isRedirectingToLogin || isLogoutRequest) {
      debugLog(
        `2. isRedirectingToLogin=${isRedirectingToLogin} || isLogoutRequest=${isLogoutRequest}`
      );
    ...
    if (
      ...
      debugLog(
        `3. !errorResponse=${!errorResponse} httpStatus=${httpStatus} && isLogoutRequest=${isLogoutRequest}`
      );
      return new Promise(() => {}); // Return unresolved promise to halt execution silently
    }
    ...
    if (
      (errorResponseCode === 'TOKEN_EXPIRED' ||
        errorResponseCode === 'REFRESH_TOKEN_EXPIRED') &&
      ...
      try {
        ...
        debugLog(
          `4. errorResponseCode=${errorResponseCode} isLogoutRequest=${isLogoutRequest} /login`
        );
        return new Promise(() => {}); // Return unresolved promise to halt execution silently
      }
    }
...
export const getServerEnvs = async (
  ...
  return await cacheWrap(async () => {
    const response = await api.get(`/envs/${plugin}/${containerName}`, {
      params,
    });
    return response.data;
  });
...
export const getConfigs = async (plugin, name) => {
  return await cacheWrap(async () => {
    ...
    debugLog(
      `api getConfigs plugin=${plugin} path=${path} response:`,
      response
    );
...
export const saveSettings = async (
  ...
  const params = { encrypted: encrypted };
  debugLog(
    `api saveSettings containerName=${containerName} jsonArrayOfObjects:`,
    jsonArrayOfObjects
  );

  return await cacheWrap(async () => {
    debugLog(
      `api.post(/settings/${plugin}/${schema}/${scope}/${containerName}`,
      jsonArrayOfObjects,
      'params:',
      { params }
    );
    ...
    debugLog(
      `api.post(/settings/${plugin}/${schema}/${scope}/${containerName} response.data:`,
      response.data
    );
    return response.data;
  });
...
export const addLogin = async (
  ...
  return await cacheWrap(async () => {
    const response = await api.put(`/logins`, {
      mailbox,
      username,
      password,
      email,
...
export const deleteLogin = async (id, alsoDeleteMailbox = false) => {
  ...
  return await cacheWrap(async () => {
    const response = await api.delete(`/logins/${id}`, {
      data: { alsoDeleteMailbox: alsoDeleteMailbox },
    });
    return response.data;
  });
};
...
export const addAccount = async (
  ...
  return await cacheWrap(async () => {
    const response = await api.post(`/accounts/${schema}/${containerName}`, {
      mailbox,
      password,
      createLogin,
    });
...
export const deleteAccount = async (
  ...
  return await cacheWrap(async () => {
    ...
    const response = await api.delete(
      `/accounts/${schema}/${containerName}/${encodeURIComponent(mailbox)}`,
      { data: { alsoDeleteLogin: alsoDeleteLogin } }
    );
    return response.data;
  });
};
...
export const addAlias = async (containerName = null, source, destination) => {
  ...
  return await cacheWrap(async () => {
    const response = await api.post(`/aliases/${containerName}`, {
      source,
      destination,
    });
    return response.data;
...
export const deleteAlias = async (
  ...
  return await cacheWrap(async () => {
    ...
    const response = await api.delete(`/aliases/${containerName}`, {
      data: { source: source, destination: destination },
    }); // regex aliases cannot be url params
    return response.data;
  });
};
