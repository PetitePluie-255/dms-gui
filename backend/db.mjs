...
import { updateAccount } from './accounts.mjs';
import { color, debugLog, errorLog, infoLog, successLog } from './backend.mjs';
  ...
  try {
    ...
    if (sql.match(/BEGIN TRANSACTION/i)) {
      ...
      if (anonParams.length) {
        debugLog(
          `${color.HIG}${color.y}DB.transaction("${sql}").run(${JSON.stringify(params)}, ${anonParams})`
        );
        ...
        debugLog(
          `${color.HIG}${color.y}DB.transaction("${sql}").run(${JSON.stringify(params)})`
        );
        insertMany = DB.transaction((params) => {
      ...
      if (anonParams.length) {
        debugLog(
          `${color.HIG}${color.y}DB.prepare("${sql}").run(${JSON.stringify(params)}, ${anonParams})`
        );
        result = DB.prepare(sql).run(params, anonParams);
      } else {
        debugLog(
          `${color.HIG}${color.y}DB.prepare("${sql}").run(${JSON.stringify(params)})`
        );
...
export const dbCount = (table, scope, schema) => {
  ...
  try {
    ...
    debugLog(
      `${color.y}DB.prepare("${sql[table].select.count}").get(${JSON.stringify(params)})`
    );
...
export const dbGet = (sql, params = {}, ...anonParams) => {
  ...
  try {
    if (sql.includes(' RETURNING ')) {
      debugLog(
        `${color.HIG}${color.y}DB.prepare("${sql}").get(${JSON.stringify(params)}, ${JSON.stringify(anonParams)})`
      );
    } else {
      debugLog(
        `${color.y}DB.prepare("${sql}").get(${JSON.stringify(params)}, ${JSON.stringify(anonParams)})`
      );
    }
...
export const dbAll = (sql, params = {}, ...anonParams) => {
  ...
  try {
    if (anonParams.length) {
      debugLog(
        `${color.y}DB.prepare("${sql}").all(${JSON.stringify(anonParams)}, ${JSON.stringify(params)})`
      );
...
export const dbInit = (DATABASE_RESET = false) => {
  if (DATABASE_RESET) {
    infoLog(
      `${color.HIG}${color.REV}start ${env.ENV_MODE}: ${color.g}${env.DATABASE} ${color.g}+RESET ${color.y}`,
      env.isDEMO ? '+DEMO' : ''
    );
    exec(`rm -f ${env.DATABASE}`);
  } else {
    infoLog(
      `${color.REV}start ${env.ENV_MODE}: ${color.g}${env.DATABASE} ${color.y}`,
      env.isDEMO ? '+DEMO' : ''
    );
  }
  if (env.isDEMO)
    exec(`cp -f ${env.DATABASE_SAMPLE} ${env.DATABASE_SAMPLE_LIVE}`); // DEMO MODE: DATABASE_RESET is implied
...
export const verifyPassword = async (password = '', login) => {
  ...
  try {
    ...
    if (password && login.salt && login.hash) {
      ...
      result.success = crypto.timingSafeEqual(
        storedHashBuffer,
        generatedHashBuffer
      );
...
export const changePassword = async (table, id, password, scope) => {
  let result;
  infoLog(
    `Call to update password for account id=${id} in table=${table} for scope=${scope}...`
  );
  ...
  try {
    ...
    debugLog(
      `Updating password for account id=${id} in table=${table} for scope=${scope}...`
    );
    result = dbRun(
      sql.logins.update.password,
      { salt: salt, hash: hash, scope: scope },
      id
    ); // doesn't hurt to add scope even when unused
    if (result.success) {
      successLog(
        `Password updated for account id=${id} in table=${table} for scope=${scope}`
      );
      return {
        success: true,
        message: `Password updated for account id=${id} in table=${table} for scope=${scope}`,
      };
    } else return result;
  } catch (error) {
    // errorLog(error.message || error);
...
export const updateDB = async (table, id, jsonDict, scope, encrypt = false) => {
  ...
  try {
    ...
    debugLog(
      `${table} ${sql[table].key}=${id} for scope=${scope}; encrypt=${encrypt}`,
      anonymizedJsonDict
    );
    ...
    for (const [key, value] of Object.entries(validDict)) {
      ...
      if (typeof value == sql[table].keys[key]) {
        // example: 0 or 1 are numbers so that's okay
        ...
        if (key == 'password') {
          ...
          if (sql[table].update[key]) {
            // sql.logins.update.isActive has {0, undefined}

            // is there a test for THAT value or ANY values?
            if (
              sql[table].update[key][value] ||
              sql[table].update[key][undefined]
            ) {
              // sql.logins.update.isActive has {0 and 1}
              ...
              if (
                ...
                if (result.success) {
                  messages.push(
                    `Updated table ${table} ${sql[table].key}=${id} with ${key}=${value}`
                  );
                  successLog(
                    `Updated table ${table} ${sql[table].key}=${id} with ${key}=${value}`
                  );
            ...
            if (encrypt) scopedValues[key] = dbEncrypt(scopedValues[key]);
            result = dbRun(
              `UPDATE ${table} set ${key} = @${key} WHERE 1=1 AND ${sql[table].key} = ?`,
              scopedValues,
              id
            );
            if (result.success) {
    ...
    return success
      ? { success: true, message: messages.join('; ') }
  ...
  try {
    ...
    if (sql[table].delete[key]) {
      ...
      if (hasTests) {
        ...
        let value2test = isNonEmptyDict(sql[table].delete[key][id])
          ? id
          : undefined;
        ...
        if (sql[table].delete[key][value2test].check(testResult.message)) {
          ...
          if (result.success) {
            successLog(
              `${table} ${key}=${id} has deleted ${result.message.changes} rows`
            );
            return {
              success: true,
              message: `${table} ${key}=${id} has deleted ${result.message.changes} rows`,
            };
          } else return result;
        } else {
          // we do not pass the test
...
export const resetTokens = async (id) => {
  ...
  try {
    ...
    if (result.success) {
      ...
      return {
        success: true,
        message: `tokens deleted for user id=${id ?? '*'}`,
      };
    } else return result;
  } catch (error) {
    // errorLog(error.message || error);
...
export const getTargetDict = (
  ...
  try {
    if (settings.length) {
      ...
      if (
        result.success &&
        result.message.length >= isNonEmptyDict(plugins[plugin][schema].keys)
      ) {
