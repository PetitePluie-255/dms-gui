...
//   moveKeyToLast,
// } from '../common.mjs'
import { reduxArrayOfObjByValue } from '../common.mjs';
import { deleteAlias, getAliases } from './aliases.mjs';
import {
  debugLog,
  errorLog,
  execDMS,
...
export const getAccounts = async (
  ...
  try {
    ...
    if (accounts.success) {
      ...
      if (refresh && containerName) {
        for (const account of accounts.message) {
          ...
          if (!result.success) {
            ...
            debugLog(
              `login id=${result.message.id} with mailbox=${result.message.mailbox} already exist`
            );

            // make sure this is the mailbox and not the username
            if (result.message.mailbox == account.mailbox) {
              debugLog(
                `updating role ${account.mailbox} to login id=${result.message.id}`
              );
              result = dbRun(
      ...
      if (roles.length)
        accounts.message = reduxArrayOfObjByValue(
          accounts.message,
          'mailbox',
          roles
        );
      ...
      if (accounts.message.length) {
        ...
        accounts.message = accounts.message.map((account) => {
          return {
            ...account,
...
export const addAccount = async (
  ...
  try {
    ...
    if (schema == 'dms')
      results = await execDMS(`email add ${mailbox} '${password}'`, targetDict);
...
export const deleteAccount = async (
  ...
  try {
    ...
    if (schema == 'dms')
      results = await execDMS(`email del -y ${mailbox}`, targetDict);
    ...
    if (!results?.returncode) {
      ...
      if (alsoDeleteLogin) {
        ...
        if (result.success) {
          // delete linked login
          if (result.message.isAccount && !result.message.isAdmin)
            result = await deleteEntry('logins', result.message.id, 'id');
...
export const updateAccount = async (
  ...
  try {
    ...
    if (schema == 'dms')
      results = await execDMS(
        `email update ${mailbox} '${password}'`,
        targetDict
      );
