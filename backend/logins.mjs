...
export const addLogin = async (
  ...
  try {
    ...
    result = dbRun(sql.logins.insert.login, {
      mailbox: mailbox,
      username: username,
      email: email,
      salt: salt,
    ...
    if (result.success) {
      successLog(
        `Saved login ${username}:${mailbox} with id=${result.message.id}`
      );
      ...
      result = dbRun(
        sql.roles.insert.role,
        { loginID: result.message.id },
        ...roles
      );
      if (result.success)
        successLog(`Saved roles for ${username}:${mailbox}: ${roles}`);
    }
    return result;
  } catch (error) {
...
export const getLogin = async (credential, withSalts = false) => {
  let login = {
    success: false,
    error: 'invalid credential: neither number/username/email/object',
  };
  try {
    // we expect either an object like {id:id}|{mailbox:mailbox}|{username:username}
    if (Number.isInteger(parseInt(credential))) {
      login = dbGet(
        sql.logins.select[withSalts ? 'loginByIdSalted' : 'loginById'],
        { [sql.logins.key]: credential }
      );
    } else if (
      typeof credential == 'string' &&
      (regexEmailStrict.test(credential) || regexUsername.test(credential))
    ) {
      login = dbGet(sql.logins.select[withSalts ? 'loginSalted' : 'login'], {
        mailbox: credential,
        username: credential,
      });
      ...
      login = dbGet(
        sql.logins.select[
          withSalts ? 'loginByObjSalted' : 'loginByObj'
        ].replace('{key}', Object.keys(credential)[0]),
        credential
      );
    }
    if (login.success) {
...
export const loginHandler = async (password, login, test) => {
  ...
  if (login.isAccount) {
    ...
    result = await doveadm(
      ...
      'loginUser',
      login.mailbox,
      { mailbox: login.mailbox, password: password }
    );
  ...
  if (result.success) {
    ...
    result = dbRun(
      sql.logins.update.resetAttempts,
    ...
    result = dbRun(
      sql.logins.update.resetAttempts,
      { attempts: currentAttempts, lockout_until: lockout_until },
      login.id
    );
...
export const loginUser = async (
  ...
  try {
      ...
      if (login.success) {
        if (login.message.lockout_until <= now) {
          if (login.message.isActive) {
            if (login.message.isAccount) {
              if (login.message.mailserver) {
                if (login.message.mailbox) {
                  ...
                  if (result.success) {
                    ...
                    message =
                      result?.error || `User ${credential} password is invalid`;
                    warnLog(message);
                    login.message = message;
                    login.success = false;
              ...
              if (result.success) {
                ...
                message =
                  test &&
                  login.message.username == 'admin' &&
                  password == 'changeme'
                    ? `Login page refresh with ${credential}/${password}`
                    : result?.error || `User ${credential} password invalid`;
                warnLog(message);
                login.message = message;
                login.success = false;
          ...
          const secondsLeft = Math.ceil(
            (login.message.lockout_until - now) / 1000
          );
          message = `Too many failed attempts. Try again in ${secondsLeft} seconds.`;
          warnLog(message);
          login.message = message;
    ...
    if (login.success) {
      const { salt, hash, attempts, lockout_until, ...cleanLogin } =
        login.message;
            ...
            if (result.success) {
              ...
              if (alsoDeleteMailbox) {
                ...
                result = await deleteAccount(
                  ...
                  login.message.mailbox
                );
              }
            }
          } // dbRun failed, return the result as is
        } else
          result = { success: false, error: `Login id=${id} does not exist` };
      } // dbGet failed, return the result as is
    } else {
      errorLog(
        'deleteLogin takes an id, arguments passed:',
        JSON.stringify(id, alsoDeleteMailbox)
      );
    ...
    if (Array.isArray(credentials) && credentials.length) {
      ...
      if (result.success) {
        ...
        logins = result.message.map((login) => {
          return { ...login, roles: JSON.parse(login.roles).filter(Boolean) };
        });
...
export const cleanRoles = (roles = [], validMailboxes = [], set = false) => {
  ...
  if (validMailboxes.size == 0) {
    ...
    // ]
    if (result.success) {
      validMailboxes = new Set(
        getAllValuesByKey(result.message, 'mailbox', true)
      );
    }
  }
...
export const getRolesFromConfigs = async (credential = null) => {
  let roles = {
    success: false,
    error: 'invalid credential: neither email/object',
  };
  try {
    ...
    // we expect either an object like {id:id}|{mailbox:mailbox}|{username:username}
    if (Number.isInteger(parseInt(credential))) {
      result = dbGet(sql.logins.select.rolesById, {
        [sql.logins.key]: credential,
      });
    } else if (
      typeof credential == 'string' &&
      regexEmailStrict.test(credential)
    ) {
      roles = dbGet(sql.logins.select.roles, {
        mailbox: credential,
        username: credential,
      });
    } else if (isNonEmptyDict(credential) == 1) {
      roles = dbGet(
        sql.logins.select.rolesByObj.replace(
          '{key}',
          Object.keys(credential)[0]
        ),
        credential
      );
    }
...
export const getRoles = async (credential) => {
  ...
  try {
    // we expect either an object like {id:id}|{mailbox:mailbox}|{username:username}
    if (Number.isInteger(parseInt(credential))) {
      roles = dbGet(sql.roles.select.rolesById, {
        [sql.roles.key]: credential,
      });
    } else if (
      typeof credential == 'string' &&
      (regexEmailStrict.test(credential) || regexUsername.test(credential))
    ) {
      roles = dbGet(sql.roles.select.roles, {
        mailbox: credential,
        username: credential,
      });
    } else if (isNonEmptyDict(credential) == 1) {
      roles = dbGet(
        sql.roles.select.rolesByObj.replace(
          '{key}',
          Object.keys(credential)[0]
        ),
        credential
